...
st.markdown("""
<style>
    /* 📱 スマホの左上メニュー（>）は残し、右上の余計なアイコン（猫マーク等）だけを消す！ */
...
# 【Ver 2.0】なんでやねんの即時反映用（押したらすぐ数字を増やすため）
...
# 👑 トレンドページを新設！
pages = [t("✍️ 投稿ルーム (自分)", "✍️ Post Room"), t("🌍 フィード (みんな)", "🌍 Global Feed"), t("👑 トレンド", "👑 Trending"), t("💬 DMチャット", "💬 DM Chat")]
page_index = 0
if "Feed" in st.session_state.current_page or "フィード" in st.session_state.current_page: page_index = 1
elif "Trending" in st.session_state.current_page or "トレンド" in st.session_state.current_page: page_index = 2
...
def render_post_card(post, is_trend=False, rank=0):
    ...
    # ローカルの「なんでやねん」カウント同期処理（押した瞬間に数を増やすため）
    ...
    if is_mine:
        ...
        with c1:
            if st.button(f"✋ {t('なんでやねん！', 'Nandeyanen!')} ({local_count})", key=f"nande_{post_id}_{is_trend}"):
                ...
                try:
                    requests.post(f"{API_URL}/api/nandeyanen/{post_id}", timeout=2)
                except:
                    pass
                st.rerun()
        ...
        with c2:
            with st.expander(t("💬 DMで直接ツッコむ", "💬 Reply in DM")):
                ...
                if st.button(f"{t1.get('emoji','')} {b1_txt}", key=f"t1_{post_id}_{is_trend}", use_container_width=True): start_dm(b1_txt); st.rerun()
                if st.button(f"{t2.get('emoji','')} {b2_txt}", key=f"t2_{post_id}_{is_trend}", use_container_width=True): start_dm(b2_txt); st.rerun()
                if st.button(f"{t3.get('emoji','')} {b3_txt}", key=f"t3_{post_id}_{is_trend}", use_container_width=True): start_dm(b3_txt); st.rerun()
...
if page_index == 0:
                ...
                if goal_input and deadline_input:
                    with st.spinner(t("考案中...", "Thinking...")):
                        try:
                            ...
                            st.session_state.yume_step = 2; st.rerun()
                        except: st.error("Error")
    ...
    if st.session_state.preview_data:
        ...
        if boke.get("type") == "yumeochi": st.markdown(f'<div class="goal-badge">🎯 {boke.get("goal")}</div>', unsafe_allow_html=True)
        ...
        with c1:
            if st.button(t("🚀 世界に放流する！", "🚀 Publish to World!"), type="primary", use_container_width=True):
                with st.spinner(t("DBに保存中...", "Saving...")):
                    requests.post(f"{API_URL}/api/publish", json={"author_id": st.session_state.user_id, "author_name": st.session_state.username, "boke_data": st.session_state.preview_data, "boke_vector": st.session_state.preview_vector})
                st.session_state.preview_data = None; st.session_state.preview_vector = None
                st.toast(t("🎉 投稿完了！", "🎉 Published!"), icon="✅")
                time.sleep(1.5); st.session_state.current_page = pages[1]; st.rerun()
        with c2:
    ...
    with st.spinner(t("最新の投稿を取得中...", "Loading latest posts...")):
        try:
            ...
            if not posts: st.info(t("まだ投稿がありません。", "No posts yet."))
                ...
                for post in posts: render_post_card(post, is_trend=False)
...
# ==========================================
    ...
    with st.spinner(t("ランキングを集計中...", "Calculating trends...")):
        try:
            trend_res = requests.get(f"{API_URL}/api/trending").json()
            posts = trend_res.get("posts", [])
            
            if not posts:
                st.info(t("まだ誰も「なんでやねん」されていません！フィードでツッコミを入れてみよう。", "No Nandeyanen yet. Go to Feed and react!"))
            else:
                # 0回以上のものだけ表示
                ...
                if not ranked_posts:
                    ...
                    for i, post in enumerate(ranked_posts[:10]): # トップ10を表示
                        render_post_card(post, is_trend=True, rank=i+1)
        except Exception as e: st.error("Error loading trends.")
